import { UserRole, ProfileType, ApplicationStatus, FavoriteType } from '@prisma/client';
...
// Set prisma to global object in development to prevent multiple instances
...
async function main() {
  ...
  try {
    ...
    // Define types for our models based on Prisma
    type User = {
      id: string;
      email: string;
      role: UserRole;
      ...
      for (let j = 0; j < numJobs; j++) {
        ...
        const jobListing = await prisma.jobListing.create({
          data: {
            ...
            tryoutInfo: `Tryouts will be held on ${new Date(Date.now() + Math.floor(Math.random() * 30) * 86400000).toISOString().split('T')[0]}`,
      ...
      for (let j = 0; j < numApplications; j++) {
        ...
        const application = await prisma.application.create({
          data: {
            ...
            coverLetter: `I am very interested in the ${jobListings[jobIndex].title} position with ${teamName}. I believe my skills and experience make me a great fit for your team.`,
          },
        });
    ...
    throw error; // Re-throw to trigger the process.exit(1) below
  }
}

// Handle the case if this file is imported vs executed directly
if (require.main === module) {
  ...
  // Export the main function for importing in other modules if needed
  module.exports = { main };
}
